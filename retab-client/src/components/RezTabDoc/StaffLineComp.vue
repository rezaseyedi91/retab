<template>
    ...
    " :class="{'ledger-line': line.isLedgerLine, 'show-ledger-line':showLedgerLine, 'italian': line.staff.getTabType() == TabType.ITALIAN}">
        ...
        <!-- <NoteComp v-for="(note, index) in tabGroup.notes.filter(n => n.course == line.courseInfo.number)" :key="index * store.state.utils.keyCoefficient"
            ...
            <!-- {{ line.isLedgerLine }} -->
        <NoteComp :tabGroup="tabGroup" :line="line" :tabType="line.staff.getTabType()"/>
    </div>
