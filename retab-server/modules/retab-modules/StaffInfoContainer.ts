import DB from "../DB";
import { TDocSettings, TInstrument, TMeiTag, TRetabDoc, TStaffInfo, TTabCourseTuningInfo } from "../db-types";
...
export default class StaffInfoContainer implements TStaffInfo {
    ...
    adjustStaffDef(el: MeiTag) {
        ...
        tuningTag.addChildren(...this.tuning.map(c => {
            const pnameAccidAttributes: MeiAttribute[] = TabNote.makeAccidedPnameAttributes(c.pname as AccidedNotePname) //            { title: 'pname', value: c.pname },
            return new Course({
    ...
    async save(docId: number) {
        ...
        const saved = await DB.getInstance().staffInfo.upsert({
            ...
            select: {
                id: true, tuning: { select: { id: true, relatedXmlId: true, n: true, oct: true, pname: true,  } }
            }
        })

        const relatedXmlIdsUpdated = await prisma.$transaction(saved.tuning.filter(s => !s.relatedXmlId)?.map(ct => {
            ct.relatedXmlId = MeiTag.prototype.generateId()
            return prisma.tabCourseTuningInfo.update({
                where: { id: ct.id },
                data: { relatedXmlId: ct.relatedXmlId },
                select: {
                    id: true, //
                    relatedXmlId: true
                }
        ...
        saved.tuning.forEach(t => {
            t.relatedXmlId = t.relatedXmlId ||  relatedXmlIdsUpdated.find(i => i.id == t.id)?.relatedXmlId!
        })
        return saved
    }
